/*
    gcc -Wall ./window_slicing.c -o window_slicing -lm -lfftw3
*/
...
/*
    ./window_slicing <frequency sampling> <frequency limit>  <window size> <window step> <raw pcm s16le samples file>
*/
int main(int argc, char** argv)
{
    ...
    for(window_offset = 0;;)
    {
        ...
        fprintf(stderr, "window_offset=%d, remains=%d... ", window_offset, samples_count - window_offset);

        for(i = 0; i < window_size && i < (samples_count - window_offset); i++)
        {
            fft_in[i][REAL] = (double)samples_buffer[window_offset + i] / 32768.0;
            fft_in[i][IMAG] = 0;
        };
